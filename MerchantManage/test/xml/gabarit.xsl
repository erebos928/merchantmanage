...
<div class="container">
<div class="row">
<div class="col-md-4 text-center"><a><img src="{Logo}" width="180" height="100" alt=""></img></a><br></br><xsl:value-of select="@preferredName"/></div>
</div>
</div>
<hr></hr>
<div class="container">
...
<xsl:variable name="identifier" select="attribute::id"/>
	<a href="?currentnode={@id}"><xsl:value-of select="$cats/entry[@key=$identifier]"></xsl:value-of></a>
</xsl:otherwise>
</xsl:choose>
...
<table>
	...
	<xsl:variable name="fid" select="attribute::id"/>
		<tr>
			<td>
				<xsl:choose>
					<xsl:when test="@preferredName"><xsl:value-of select="@preferredName"/></xsl:when>
					<xsl:otherwise><a href="{$feats/entry[@key=$fid]}"/></xsl:otherwise>
				</xsl:choose>	
			</td>
		</tr>
	</xsl:for-each>
